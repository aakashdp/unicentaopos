...
// JG uniCenta June 2014 includes StockUnits 
        ...
        productsRow = new Row(
                ...
                new Field("DEPOSIT", Datas.INT, Formats.INT)
...
// JG Oct 2013 - add for CustomerView>Tranx table
    ...
    public final List<CustomerTransaction> getCustomersTransactionList() throws BasicException {
        return new PreparedSentence(s,               
                ...
                + "SUM(TICKETLINES.UNITS * TICKETLINES.DEPOSIT ) AS DEPOSIT"        
                + "SUM(TICKETLINES.UNITS * (TICKETLINES.PRICE * (1.0 + TAXES.RATE) + TICKETLINES.DEPOST )) AS TOTAL, "
 ...
 public void writeValues() throws BasicException {
        ...
        if (ticket != null) {
            ...
            ticket.setLines(new PreparedSentence(s
                , "SELECT L.TICKET, L.LINE, L.PRODUCT, L.ATTRIBUTESETINSTANCE_ID, L.UNITS, L.PRICE, L.DEPOSIT, T.ID, T.NAME, T.CATEGORY, T.CUSTCATEGORY, T.PARENTID, T.RATE, T.RATECASCADE, T.RATEORDER, L.ATTRIBUTES " +
...
public Object transact() throws BasicException {
    ...
    SentenceExec ticketlineinsert = new PreparedSentence(s
        , "INSERT INTO TICKETLINES (TICKET, LINE, PRODUCT, ATTRIBUTESETINSTANCE_ID, UNITS, PRICE, DEPOSIT, TAXID, ATTRIBUTES) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
			...
			if (i > 0) {
				if (((Boolean)values[26])) {
					if (new PreparedSentence(s
							...
							, new SerializerWriteBasicExt(productsRow.getDatas(), new int[] {27, 0})).exec(params) == 0) {
						new PreparedSentence(s
							, "INSERT INTO PRODUCTS_CAT (PRODUCT, CATORDER) VALUES (?, ?)"
							, new SerializerWriteBasicExt(productsRow.getDatas(), new int[] {0, 27})).exec(params);
					}
