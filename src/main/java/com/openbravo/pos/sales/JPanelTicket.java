...
import com.openbravo.pos.inventory.BottleDeposit;
...
//   private String loyaltyCardNumber=null;
    ...
    public void init(AppView app) throws BeanFactoryException {
        ...
        depositRate = (new BottleDeposit(dlSystem)).getDepositRate();
...
// if the delay period is not zero create a inactivitylistener instance        
        if (delay != 0){
            listener = new InactivityListener(logout1,delay); 
            listener.start();
        } 
...
//            resetSouthComponent();
    ...
    private void printPartialTotals(){
        ...
        if (m_oTicket.getLinesCount() == 0) {
            ...
            m_jDepositEuros.setText(m_oTicket.printDeposit());
...
//        oProduct.setTaxCategoryID(((TaxCategoryInfo) taxcategoriesmodel.getSelectedItem()).getID()); 
        ...
        if (oProduct.isVprice()){
            ...
            if (m_jaddtax.isSelected()) {
                ...
                addTicketLine(new TicketLineInfo(oProduct, dMul, dPrice, oProduct.getDepositCount()*depositRate, tax, (java.util.Properties) (oProduct.getProperties().clone())));
        } else {        
                TaxInfo tax = taxeslogic.getTaxInfo(oProduct.getTaxCategoryID(), m_oTicket.getCustomer());
                addTicketLine(new TicketLineInfo(oProduct, dMul, dPrice, oProduct.getDepositCount()*depositRate, tax, (java.util.Properties) (oProduct.getProperties().clone())));                
            }
    ...
    private void stateTransition(char cTrans) {
        ...
        if ((cTrans == '\n') || (cTrans == '?')) {
            if (m_sBarcode.length() > 0) { 
                ...
                if (sCode.startsWith("c")) { // CUSTOMER card
                    try {
...
// VARIABLE WEIGHT BY PRICE
                            switch (sVariableTypePrefix) {
                                case "20": dPriceSell = Double.parseDouble(sVariableNum) / 100; // price with two decimals
                                    break;                                    
                                case "02": dPriceSell = Double.parseDouble(sVariableNum) / 100; // price with two decimals
                                    break;
                                case "21": dPriceSell = Double.parseDouble(sVariableNum) / 10; //price with one decimal
                                    break;
                                case "22": dPriceSell = Double.parseDouble(sVariableNum); //price with no decimals
                                    break;
// VARIABLE PRICE BY WEIGHT
                                case "23": weight = Double.parseDouble(sVariableNum) / 1000; //weight in kg with three decimals (eg. 1,234 kg)
                                    break;
                                case "24": weight = Double.parseDouble(sVariableNum) / 100; //weight in kg with two decimals (eg. 12,34 kg)
                                    break;
                                case "25": weight = Double.parseDouble(sVariableNum) / 10; //weight in kg with one decimal (eg. 123,4 kg)
                                    break;
                                case "28": sVariableNum = sCode.substring(7, 12);
...
// VARIABLE WEIGHT BY PRICE
                            double dPriceSell = Double.parseDouble(sVariableNum) / 100; // price with two decimals
            ...
            if (cTrans == '\u007f') { 
                ...
                m_jPrice.setText(Character.toString('0'));
                    ...
                    && (m_iNumberStatus == NUMBER_INPUTZERO)) {
                // Use 02 to identify Product Price Net of Tax
                if (!priceWith00){
            ...
            } else if ((cTrans == '0') && (m_iNumberStatus == NUMBER_INPUTZERODEC 
                    || m_iNumberStatus == NUMBER_INPUTDEC)) {
            ...
            } else if ((cTrans == '0') && (m_iNumberStatus == NUMBER_PORZERODEC 
                    || m_iNumberStatus == NUMBER_PORDEC)) {
                // Un numero decimal
                m_jPor.setText(m_jPor.getText() + cTrans);
            } else if ((cTrans == '1' || cTrans == '2' || cTrans == '3' 
...
/** This method is called from within the constructor to
    ...
    private void initComponents() {
        ...
        m_jDepositEuros = new javax.swing.JLabel();
        ...
        j_btnKitchenPrt.setEnabled(false);
        ...
        m_jLblTotalEuros5.setText(AppLocal.getIntString("label.deposit")); // NOI18N
        ...
        m_jDepositEuros.setBackground(m_jEditLine.getBackground());
        m_jDepositEuros.setFont(new java.awt.Font("Arial", 0, 18)); // NOI18N
        m_jDepositEuros.setForeground(m_jEditLine.getForeground());
        m_jDepositEuros.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
        m_jDepositEuros.setLabelFor(m_jTaxesEuros);
        m_jDepositEuros.setBorder(new javax.swing.border.LineBorder(new java.awt.Color(153, 153, 153), 1, true));
        m_jDepositEuros.setMaximumSize(new java.awt.Dimension(125, 25));
        m_jDepositEuros.setMinimumSize(new java.awt.Dimension(80, 25));
        m_jDepositEuros.setOpaque(true);
        m_jDepositEuros.setPreferredSize(new java.awt.Dimension(80, 25));
        m_jDepositEuros.setRequestFocusEnabled(false);
        m_jPanTotals.add(m_jDepositEuros);
...
// Autologoff after sending to kitchen   
    ...
    private javax.swing.JLabel m_jDepositEuros;
